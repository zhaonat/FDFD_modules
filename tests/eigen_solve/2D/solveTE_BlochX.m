function [Hz, Ex, Ey, omega_eigs,A] = solveTE_BlochX(L0, wvlen_center, xrange, yrange, eps_r, Kx, Npml, n)
%% Input Parameters
...
A = -mu0^-1 * (Dxf*T_eps_x^-1*Dxb - 2*1i*Kx * Vxf*T_eps_x^-1*Dxb -...
    Kx^2*T_eps_z^-1 + Dyf*T_eps_y^-1*Dyb); 
