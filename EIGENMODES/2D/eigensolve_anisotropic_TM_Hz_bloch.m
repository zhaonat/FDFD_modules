...
function [eigenvals, eigenmodes,A] = eigensolve_anisotropic_TM_Hz_bloch(L0, wvlen, xrange, ...
    ...
    exy = bwdmean_w(bwdmean_w( exy, 'y'),'x');  % average eps for eps_x
    eyx = bwdmean_w(bwdmean_w(eyx, 'x'), 'y');  % average eps for eps_y
    ...
    K = [Kx,0];
    Dxf = createDws_bloch('x', 'f', dL, N, K, L);
    Dyf = createDws_bloch('y', 'f', dL, N, K, L);
    Dyb = createDws_bloch('y', 'b', dL, N, K, L);
    Dxb = createDws_bloch('x', 'b', dL, N, K, L);
