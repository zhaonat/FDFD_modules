...
omega_scan = linspace(1e14, omega_p, 100);
figure();
...
%     drawnow();
    ...
    for i = 1:length(filtered_eigs)
        ...
        % impose the condition that if imag(kx) is too large, then it's not
        % really a mode of the system
        if(abs(imag(filtered_eigs(i)) > abs(real(filtered_eigs(i)))))
            continue
        else
            subplot(121)
            plot(real(filtered_eigs(i))*(L(1)/(2*pi)), omega,'.b')
            hold on;

            plot(kx_scan*(L(1)/(2*pi)), omega_light)
            ylim([0, max(omega_scan)])
            subplot(122)
            plot(imag(filtered_eigs(i))*(L(1)/(2*pi)), omega,'.r')
            hold on;
        end
    end
    drawnow();
