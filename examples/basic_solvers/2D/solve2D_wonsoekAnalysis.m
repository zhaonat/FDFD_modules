...
xrange = 5*[-1 1]*L0;  % x boundaries in L0
yrange = 5*[-1 1]*L0;  % y boundaries in L0
Nx = 100; Ny = Nx;
N = [Nx Ny];  % [Nx Ny]
...
%     leftop = 10^7*Tmz^-1*(Dxf+Dyf);
%     rightop = 10^7*(1i/omega)*Tmz^-1*(Dxf+Dyf)*(Jz);
    %% Construct the matrix A, everything is in 2D
    A = Dxb*(Tmy^-1)*Dxf + Dyb*(Tmx^-1)*Dyf + omega^2*Tepz;%leftop;
