function [U, Farfield, total_power] = Plot_Farfield_Pattern_II(r_near, norm_surf, E_near,...
...
%     Ht_near = H_near(1:2, :);
    ...
    for ind_theta = 1:length(theta_table)
        ...
        Ez_far = NTFF_Transform_2D_II(E_near, H_near, norm_surf, r_near, dl, r_hat, wavelength, polarization);
...
%     Hz_near = H_near(3, :);
    ...
    for ind_theta = 1:length(theta_table)
        theta = theta_table(ind_theta);
        r_hat = [cos(theta); sin(theta)];
        Hz_far = NTFF_Transform_2D_II(E_near, H_near, norm_surf, r_near, dl, r_hat, wavelength, polarization);
...
%is this actually a directivity measurement?
% I don't think so...because it really isn't a comparison against the
% antenna in isolated medium...
