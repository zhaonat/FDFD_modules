function [r_near, norm_surf, E_near, H_near] =...
    Get_Nearfield_2D_II(box_bound, E, H, xrange, yrange, dL, side_names, polarization)
...
%         
mask_flag{1} = up_flag*ones(size(x_up));
mask_flag{2} = left_flag*ones(size(y_left));
mask_flag{3} = down_flag*ones(size(x_down));
mask_flag{4} = right_flag* ones(size(y_right));
%mask_flag is 1x4*
...
% since up and down correspond to dx
% left and right correspond to dy
...
norm_surf{1} = [zeros(size(x_up)); ones(size(x_up))];
norm_surf{2}= [-1*ones(size(y_left)); zeros(size(y_left))];
norm_surf{3}= [zeros(size(x_down)); -1*ones(size(x_down))];
norm_surf{4}= [ones(size(y_right)); zeros(size(y_right))];
%% get field values at sampling positions
if polarization == 's'
    for i = 1:4
        Ez{i} = interp2(sx_prime, sy_prime, E, r_near{i}(1,:), r_near{i}(2,:));      % Ez field at sampling points
        Hx{i} = interp2(sx_prime, sy_dual, H{1}, r_near{i}(1,:), r_near{i}(2,:));   % Hx
        Hy{i} = interp2(sx_dual, sy_prime, H{2}, r_near{i}(1,:), r_near{i}(2,:));   % Hy
       
        E_near{i} = [zeros(size(Ez{i})); zeros(size(Ez{i})); mask_flag{i}.* Ez{i}];
        % see Hx, Hy, Hz...which is 0 for 's'
        H_near{i} = [mask_flag{i}.* Hx{i}; mask_flag{i}.* Hy{i}; zeros(size(Hx{i}))];
    end

elseif polarization == 'p'
    % r_near(1,:) = Xq; r_near(2,:) = Yq;
    for i = 1:4
        Ex{i} = interp2(sx_dual, sy_prime, E{1}, r_near{i}(1,:), r_near{i}(2,:));   % Ex
        Ey{i} = interp2(sx_prime, sy_dual, E{2}, r_near{i}(1,:), r_near{i}(2,:));   % Ey
        Hz{i} = interp2(sx_dual, sy_dual, H, r_near{i}(1,:), r_near{i}(2,:));      % Hz
        E_near{i} = [mask_flag{i}.* Ex{i}; mask_flag{i}.* Ey{i}; zeros(size(Ex{i}))];
        H_near{i} = [zeros(size(Hz{i})); zeros(size(Hz{i})); mask_flag{i}.* Hz{i}];
    end
    % Ex, Ey, Hz (which is 0 for P polarization)
    % 
